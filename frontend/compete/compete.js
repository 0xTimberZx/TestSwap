...
async function updateCostDisplay() {
  ...
  if (extraRounds > 0) {
    try {
      ...
      baseCost += ` + ${fmtTIMBS(extraCost)} (extra rounds)`;
    } catch {}
  }

  el.textContent = baseCost;
}
...
// ─── Cost Mode + Extra Rounds ─────────────────────────────────────────────────

function setCostMode(mode) {
  costMode = mode;
  document.getElementById("cost-eth-btn").classList.toggle("active", mode === "eth");
  document.getElementById("cost-timbs-btn").classList.toggle("active", mode === "timbs");
  updateCostDisplay();
}
...
async function handleSubmitEntry() {
  ...
  try {
    ...
    const gas = await getGasParams();
    const nonce = await getPendingNonce();
    const value = useETH ? entryCostETH_wei : 0;
